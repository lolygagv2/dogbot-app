...
- [x] **DATA REFACTOR CLOSED (robot dad2a97, 2026-09-01): Phase 4 complete.** Legacy writers off, spec rows single source. App verified safe against all wire deltas: /sg/sessions/recent + /events/dog* have zero app consumers; sg_summary.session_id unused by app and _asString() is int/string-lenient → told robot to RETIRE the int shim. Fleet historical data consolidated to a deduped analysis archive (Morgan holds evidence pack).
