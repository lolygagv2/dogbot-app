...
2. Check if large messages are causing issues
...
# Handle large messages better
MAX_MESSAGE_SIZE = 20 * 1024 * 1024  # 20MB for MP3 uploads
```

---

### P2: Event Forwarding Verification

Ensure all these events flow through:
- `mission_progress` 
- `mode_changed`
- `dog_detected`
- `treat_dispensed`

Add logging to confirm events are being forwarded.
...
| Connection stability | Relay | Random disconnects |
...
| Event forwarding logs | Relay | Verify events flow through |
